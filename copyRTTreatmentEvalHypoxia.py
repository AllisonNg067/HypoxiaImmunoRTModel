...
IT = (True, True)
RT_fractions = 1
#alpha and bfrom fit without hypoxia to get identical schedules to non hypoxia model
param = [0, 0, 0.15769230769230763, 0.04269230769230769]
bed = 80
initial_cell_count = 100000
file_name = 'new smaller sampling range matched params hypoxia RT ' + str(RT_fractions) + ' fraction a.csv'
...
# Load from CSV
params = pd.read_csv('new_matched_hypoxia_parameters.csv').values.tolist()
#print(params)
sample_size = 10
...
def trial_treatment(i, file):
  ...
  treatment_res_list = [t_rad, D, t_treat_p1, 0, t_treat_c4, 0, np.mean(TCPs), np.mean(times), TCPs, times]
