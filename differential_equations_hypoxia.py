...
# initialise all the arrays to have the initial conditions
    ...
    while j <= m-1:
           ...
           #print(C[:,j])
            ...
            for ii in range(d):
              # C_kin is the -omega function (natural clearing), im_death_d is immune cell death
                ...
                im_death_dN = immune_death_dePillis(C_tot_N[:, j], (1-u)*Ta_tum[:, j], p, q, s, p1, p_11, mi, vol_flag, time_flag, time[j+1], t_treat_p1[ind_p11], delta_t)[0]
                ...
                if C_tot_H[:, j+1] == 0:
                    ...
                    im_death_dH = immune_death_dePillis(C_tot_H[:, j], u*Ta_tum[:, j], p, q, s, p1, p_11, mi, vol_flag, time_flag, time[j+1], t_treat_p1[ind_p11], delta_t)[0]
        ...
        if time[j-1] > t_eq + t_f2 and vol_flag == 1:
            ...
            #print('last time', time[-1])
            return vol, radius, radius_H, t_eq, time, C_tot, C, C_dam, C_tot_N, C_tot_H, C_N, C_H, C_dam_N, C_dam_H, A, Ta_tum, T_lym, Ta_lym, Tb_lym, c4_list
...
# delta_t = 0.05
# D = [5,5]
# t_rad = np.array([10,11])
# t_treat_c4 = np.array([10])
# t_treat_p1 = np.array([10])
# c4 = 2
# p1 = 0.2
...
# 0.0,
# 0.10000000000000002,
# 2.2458318956090505e+80]
# param[24] = 0  # ctla4 dose
# param[37] = 0 #pd dose
# # param[24] = 0.0005  # ctla4 decay rate
# # param[25] = 10**-5
# # param[-1] = 10**20
#vol, radius, radius_H, _, time, C_tot, C, C_dam, C_tot_N, C_tot_H, C_N, C_H, C_dam_N, C_dam_H, A, Ta_tum, T_lym, Ta_lym, Tb_lym, c_4 = radioimmuno_response_model(param, 0.05, free, t_f1, t_f2, D, t_rad, t_treat_c4, t_treat_p1, LQL, activate_vd, use_Markov)
#print(radius[0])
#print(C_H[0])
