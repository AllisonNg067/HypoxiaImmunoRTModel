...
file_name = 'smaller sampling range hypoxia RT ' + str(RT_fractions) + ' CTLA4 ' + str(CTLA4_fractions) + ' a.csv'
schedule_list, DList = get_treatment_and_dose(bed, RT_fractions, param, 0, CTLA4_fractions)
#print('Dlist', DList)
#print(schedule_list)
# paramNew = list(param)
...
def evaluate_patient(i, t_rad, t_treat_p1, t_treat_c4, D):
  ...
  paramNew = params[i]
  paramNew[24] = 0.8/CTLA4_fractions
  paramNew[37] = 0
    #print(paramNew)
...
def trial_treatment(i, file):
  ...
  treatment_res_list = [t_rad, D, t_treat_p1, 0, t_treat_c4, 0.8/CTLA4_fractions, np.mean(TCPs), np.mean(times), TCPs, times]
